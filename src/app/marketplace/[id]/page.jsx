...
export default function MaterialDetailsPage() {
	...
	//useEffect to trigger logic on page load to increment view count and fetch updated stats
	useEffect(() => {
  const loadViews = async () => {
try{
	//POST endpoint to increment the viewcount for this material
    await fetch(`http://localhost:4000/api/materials/${id}/view`, {
      method: "POST"
    });

	//GET endpoint to fetch updated views
    const res = await fetch(`http://localhost:4000/api/materials/${id}/stats`,{
		cache: "no-store"
	});
    const data = await res.json();
	console.log("View stats:", data);
